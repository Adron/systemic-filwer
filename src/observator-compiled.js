...
var loggit = function loggit(appendData) {
...
observator.watch = function () {
  ...
  watcher = chokidar.watch(pathToWatch, options).on("all", eventAll).on("add", eventAll).on("change", eventAll).on("unlink", eventAll).on("addDir", eventAll).on("unlinkDir", eventAll).on("ready", eventAll).on("raw", eventAll).on("error", eventAll);
};
...
//# sourceMappingURL=observator-compiled.js.map